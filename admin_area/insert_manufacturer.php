...
?>
    ...
    if (isset($_POST['submit'])) {
        ...
        $image_name = uniqid() . '_' . $_FILES['manufacturer_image']['name'];
        $tmp_name = $_FILES['manufacturer_image']['tmp_name'];
        $upload_path = "other_images/$image_name";

        if (!empty($tmp_name)) { // Check if file is uploaded
            if (move_uploaded_file($image_name, $upload_path)) {
                // Check for file upload errors
                if ($_FILES['manufacturer_image']['error'] !== UPLOAD_ERR_OK) {
                    echo "Upload failed with error code: " . $_FILES['manufacturer_image']['error'];
                } else {
                    $insert_manufacturer = "INSERT INTO manufacturers (manufacturer_title, manufacturer_top, manufacturer_image) VALUES ('$manufacturer_name', '$manufacturer_top', '$image_name')";
                    ...
                    if ($run_manufacturer) {
                        ...
                        echo "<script>alert('Error inserting manufacturer')</script>";
                    }
                }
            } else {
